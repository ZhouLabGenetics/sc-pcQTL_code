#!/bin/bash
# Submit SC cluster identification jobs for chromosomes 1-22
...
for chr in ${CHRS}; do
  sbatch --job-name=sccl_${CELL_TYPE}_chr${chr} \
         --output="${LOG_DIR}/step3_chr${chr}_${CELL_TYPE}_%j.out" \
         --error="${LOG_DIR}/step3_chr${chr}_${CELL_TYPE}_%j.err" \
         --time=04:00:00 \
         --mem=16G \
         --cpus-per-task=1 \
         --wrap="cd ${SCRIPT_DIR} && ${RUN_R} step3_identify_clusters_single_chr.R ${chr}"
done
